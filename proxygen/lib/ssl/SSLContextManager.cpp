...
do { \
  throw proxygen::Exception("missing " #name " support in openssl"); \
} while(0)
 ...
 *    It maps to Accetpor in the proxygen vocabulary.
 *
...
void SSLContextManager::addSSLContextConfig(
  ...
  for (const auto& cert : ctxConfig.certificates) {
    ...
    if (!cn) {
      throw Exception(folly::to<string>("Cannot get CN for X509 ",
                                        cert.certPath));
    }
    ...
    if (numCerts == 1) {
      ...
      if (commonName != *cn) {
        throw Exception(folly::to<string>("X509 ", cert.certPath,
  ...
  // Let proxygen pick the highest performing cipher from among the client's
  // choices.
  //
  ...
  // applications disable them (it's part of SSL_OP_ALL).  For proxygen this
...
SSLContextManager::ctxSetupByOpensslFeature(
 ...
 sslCtx->getSSLCtx()->mode |= SSL_MODE_RELEASE_BUFFERS;
  ...
  if (ctxConfig.isDefault) {
    if (defaultCtx_) {
      throw Exception(">1 X509 is set as default");
    }
...
SSLContextManager::insert(shared_ptr<SSLContext> sslCtx,
  ...
  if (cn->length() == 1 && (*cn)[0] == '*') {
    if (!defaultFallback) {
      throw Exception("STAR X509 is not the default");
    }
                                                ...
                                                shared_ptr<SSLContext> sslCtx)
{
  ...
  if (len > 2 && dn[0] == '*') {
    if (dn[1] == '.') {
      ...
      throw Exception("Invalid wildcard CN/subject-alternative-name \"%s\" "
  ...
  if (len == 1 && *dn == '.') {
    throw Exception("X509 has only '.' in the CN or subject alternative name "
                    "(after removing any preceding '*')");
  }

  if (strchr(dn, '*')) {
    throw Exception("X509 has '*' in the the CN or subject alternative name "
                    "(after removing any preceding '*')");
  }
  ...
  if (v == dnMap_.end()) {
    ...
    throw Exception("Duplicate CN or subject alternative name found: \"",
                    dnstr.c_str(), "\"");
  }
}
