...
struct TransportInfo {
   ...
   * 3. The calculation in point (2) is typically done outside proxygen
   *
   * Future plan:
   * We should log the timestamps (TimePoints) and allow
   * the consumer to calculate the latency whenever it
   * wants instead of calculating them in proxygen, for the sake of flexibility.
   * For example:
