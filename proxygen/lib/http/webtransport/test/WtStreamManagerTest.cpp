...
class WtStreamManagerTest : public ::testing::Test {
 public:
  // reinitializes WtStreamManager with a given WtConfig
  void reinit(WtConfig config) {
    streamManager.emplace(
  ...
  std::unique_ptr<quic::HTTPPriorityQueue> priorityQueue{
      std::make_unique<quic::HTTPPriorityQueue>()};
...
TEST_F(WtStreamManagerTest, BasicSelfBidi) {
  config = {.peerMaxStreamsBidi = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, BasicSelfUni) {
...
TEST_F(WtStreamManagerTest, BasicPeerBidi) {
  config = {.selfMaxStreamsBidi = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, BasicPeerUni) {
  config = {.selfMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, NextBidiUniHandle) {
  config = {.peerMaxStreamsBidi = 2, .peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, StreamLimits) {
...
TEST_F(WtStreamManagerTest, EnqueueIngressData) {
  config = {.peerMaxStreamsBidi = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, EnqueueIngressDataRwnd) {
  config = {.peerMaxStreamsBidi = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, WriteEgressHandle) {
  config = {.peerMaxStreamsBidi = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, GrantFlowControlCredit) {
  config = {.peerMaxStreamsBidi = 2};
  reinit(config);
  ...
  EXPECT_TRUE(streamManager->enqueue(*two.readHandle,
                                     {makeBuf(kBufLen), /*fin=*/true}));
...
TEST_F(WtStreamManagerTest, GrantConnFlowControlCreditAfterRead) {
  config = {.peerMaxStreamsBidi = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, NonDefaultFlowControlValues) {
...
TEST_F(WtStreamManagerTest, BidiStreamDataLimitsFollowStreamInitiator) {
  ...
  EXPECT_FALSE(streamManager->enqueue(*selfBidi.readHandle,
                                      {makeBuf(1), /*fin=*/false}));
  EXPECT_TRUE(streamManager->enqueue(*peerBidi.readHandle,
                                     {makeBuf(kSelfRemote), /*fin=*/false}));
  EXPECT_FALSE(streamManager->enqueue(*peerBidi.readHandle,
                                      {makeBuf(1), /*fin=*/false}));
}

TEST_F(WtStreamManagerTest, ResetStreamReleasesConnFlowControl) {
  config = {.selfMaxStreamsUni = 10};
  reinit(config);
  constexpr auto kBufLen = 65'535;
...
TEST_F(WtStreamManagerTest, StopSendingResetStreamTest) {
  constexpr auto kBufLen = 65'535;
...
TEST_F(WtStreamManagerTest, StopSendingTest) {
...
TEST_F(WtStreamManagerTest, AwaitWritableTest) {
  constexpr auto kBufLen = 65'535;
  // next ::createBidiHandle should succeed
...
TEST_F(WtStreamManagerTest, WritableStreams) {
  config = {.peerMaxStreamsUni = 3};
  reinit(config);
  constexpr auto kBufLen = 65'535;
...
TEST_F(WtStreamManagerTest, DrainWtSession) {
  config = {.peerMaxStreamsBidi = 2, .peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, CloseWtSession) {
...
TEST_F(WtStreamManagerTest, ResetStreamReliableSize) {
...
TEST_F(WtStreamManagerTest, InvalidCtrlFrames) {
  auto bidi = streamManager->createBidiHandle();
...
TEST_F(WtStreamManagerTest, IssueMaxStreamsBidiUni) {
  config = {.selfMaxStreamsBidi = 2, .selfMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, ShutdownOpenStreams) {
  // create bidi stream
  auto bidi = streamManager->createBidiHandle();
...
TEST_F(WtStreamManagerTest, NoopNonExistentStreams) {
  config = {.peerMaxStreamsBidi = 2, .peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, OnlyFinPending) {
  config = {.peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, OnlyFinPendingFcBlocked) {
  config = {
      .peerMaxStreamsUni = 3,
      .peerMaxConnData = 1,
...
TEST_F(WtStreamManagerTest, NextWritableReturnsNullptrWhenQueueEmpty) {
  config = {.peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, NextWritableReturnsNullptrWhenHeadIsDatagram) {
  config = {.peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, PerStreamCallbacks) {
  auto bidi = streamManager->createBidiHandle();
  CHECK(bidi.readHandle && bidi.writeHandle);
...
TEST_F(WtStreamManagerTest, DeliveryCallback) {
  config = {.peerMaxStreamsUni = 5};
  reinit(config);
...
TEST_F(WtStreamManagerTest, ByteEventCancellation) {
  config = {.peerMaxStreamsUni = 2};
  reinit(config);
...
TEST_F(WtStreamManagerTest, CannotOpenPreviouslyClosedStreams) {
  config = {.peerMaxStreamsUni = 100};
  reinit(config);
...
TEST_F(WtStreamManagerTest, UserHandlesDestroyedBeforeManager) {
...
TEST_F(WtStreamManagerTest, ManagerDestroyedBeforeUserHandles) {
...
TEST_F(WtStreamManagerTest, UserHandlesTest) {
  { // bidi tests
...
TEST_F(WtStreamManagerTest, FlowControlInfo) {
