...
class QCRAMQueue : public folly::DestructorCheck {
 public:
  explicit QCRAMQueue(QCRAMCodec& codec)
    : codec_(codec) {}
  ...
  void enqueueHeaderBlock(uint32_t seqn,
    ...
    if (!flags.QCRAMPrefixHasDepends || codec_.dependsOK(depends)) {
      ...
      if (decodeBlock(seqn, flags, std::move(block), length, streamingCb,
                      false /* in order */)) {
        return;
      }
      drainQueue();
    ...
    VLOG(1) << "HoL blocked for seqn " << seqn << " depends " << depends << " length " << length;
    queuedBytes_ += length;
    holBlockCount_++;
    Blocked blocked{.seqn = seqn, .flags = flags, .block = std::move(block),
          .length = length, .streamingCb = streamingCb};
 ...
 private:
  ...
  bool decodeBlock(int32_t seqn, FrameFlags flags,
                   std::unique_ptr<folly::IOBuf> block,
  ...
  void drainQueue() {
    ...
    while (it != blocked_.end()) {
      ...
      if (!codec_.dependsOK(depends)) {
        VLOG(1) << "Still HoL blocked seqn " << blocked.seqn << " depends " << depends;
        return;
      }
      VLOG(1) << "draining blocked size " << blocked_.size();
      VLOG(1) << "HoL unblocked seqn " << blocked.seqn << " depends " << depends << " length " << blocked.length;
      DCHECK_LE(blocked.length, queuedBytes_);
      queuedBytes_ -= blocked.length;
      decodeBlock(blocked.seqn, blocked.flags, std::move(blocked.block),
                  blocked.length, blocked.streamingCb, false);
