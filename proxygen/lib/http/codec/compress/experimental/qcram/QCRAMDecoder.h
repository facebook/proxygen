...
//#include
//<proxygen/lib/http/codec/compress/experimental/simulator/CompressionTypes.h>
#include <proxygen/lib/http/codec/compress/HPACKDecodeBuffer.h>

namespace proxygen {

struct FrameFlags {
  FrameFlags(bool a, bool b) : allowOOO(a), QCRAMPrefixHasDepends(b) {
  }
  FrameFlags() = default;
  bool allowOOO{false};
...
class MembershipRange {
 public:
  MembershipRange(size_t l, size_t h)
      : low(l), high(h) {}
...
struct cmpByLow
    : public std::binary_function<MembershipRange, MembershipRange, bool> {
  bool operator()(const MembershipRange& lhs,
                  const MembershipRange& rhs) const {
...
class QCRAMDecoder : public QCRAMContext {
 public:
  ...
  using headers_t = std::vector<QCRAMHeader>;
