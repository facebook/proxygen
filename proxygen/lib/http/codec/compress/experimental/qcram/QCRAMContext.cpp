...
QCRAMContext::QCRAMContext(uint32_t tableSize, bool qcram, bool useBaseIndex) :
...
uint32_t QCRAMContext::getIndex(const HPACKHeader& header,
  ...
  uint32_t dynamicIndex = table_.getIndex(header, allowVulnerable, commitEpoch, curEpoch);
...
bool QCRAMContext::isVulnerable(uint32_t index,
                                int32_t commitEpoch,
                                int32_t packetEpoch) {
  if (isStatic(index)) return false;
  uint32_t dynamicIndex = globalToDynamicIndex(index);
  bool result = table_.isVulnerableEpoch(dynamicIndex, commitEpoch, packetEpoch);
  if (result) {
...
void QCRAMContext::seedHeaderTable(
