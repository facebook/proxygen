...
class QCRAMDeduplicationTable {
  ...
  std::pair<std::shared_ptr<folly::fbstring>, uint32_t> add(const folly::fbstring& s) {
    uint32_t size = 0;
 ...
 private:
  ...
  std::unordered_map<std::string, std::weak_ptr<folly::fbstring>,
                     std::hash<std::string>> dedupTable_;
};
...
class QCRAMNewTableImpl {
 public:
  ...
  void add(size_t head, const HPACKHeaderName& name,
           const folly::fbstring& value, int32_t epoch,
           int32_t packetEpoch);
  bool isVulnerableEpoch(uint32_t i, int32_t commitEpoch,
...
class QCRAMHeaderTable {
 public:
  ...
  QCRAMHeaderTable(std::unique_ptr<QCRAMNewTableImpl> table, uint32_t capacityVal)
  ...
  bool isVulnerableEpoch(uint32_t index, int32_t commitEpoch,
                         int32_t packetEpoch) const {
    return table_->isVulnerableEpoch(toInternal(index),
                                     commitEpoch, packetEpoch);
  }
  ...
  */
  ...
  uint32_t writeBaseIndex() const { return writeBaseIndex_;  }
