...
void QCRAMDecoder::updateMembership() {
  ...
  MembershipRange singleton{.low = index, .high= index};
  ...
  if (it != membership_.end()) {
    ...
    if (right.low == singleton.low + 1) {
      ...
      membership_.insert(singleton);
      VLOG(1) << "membership: merge with right new range [" << singleton.low << "," << singleton.high << "]";
    }
  }
  it--;
  auto left = *it;
  ...
  if (left.high == singleton.low - 1) {
    ...
    DCHECK_GT(before, membership_.size());
    membership_.insert(singleton);
    VLOG(1) << "membership: merge with left new range [" << singleton.low << "," << singleton.high << "]";
    merged = true;
  }
  if (merged) return;
...
bool QCRAMDecoder::dependsOK(uint32_t depends) const {
  if (membership_.empty()) return false;
