...
class QCRAMNewScheme : public CompressionScheme {
 public:
  explicit QCRAMNewScheme(CompressionSimulator* sim)
  ...
  std::pair<FrameFlags, std::unique_ptr<folly::IOBuf>> encode(
    ...
    struct FrameFlags flags{.allowOOO = true,
          .QCRAMPrefixHasDepends = (result.streamDepends != kMaxIndex) };
  ...
  void decodeControl(std::unique_ptr<folly::IOBuf> buf, SimStats& stats) {
    ...
    len -= sizeof(uint16_t);
    struct FrameFlags flags{.allowOOO = true, .QCRAMPrefixHasDepends = (seqn > 0)};
    serverQueue_.enqueueHeaderBlock(seqn, std::move(buf), len, nullptr, flags, depends);
  ...
  void decode(FrameFlags flags, std::unique_ptr<folly::IOBuf> encodedReq,
    ...
    if (sEnableUpdatesOnControlStream_) {
      ...
      DVLOG(1) << "read control len " << controlLen << " pop " << encodedReq->length();
      ...
      if (controlLen > 0) {
        ...
        while (controlBuffer->computeChainDataLength() < controlLen) {
          ...
          VLOG(1) << "control buf chain len " << controlBuffer->computeChainDataLength();
        }
  ...
  QCRAMCodec client_{TransportDirection::UPSTREAM, true, true, false};
  QCRAMCodec server_{TransportDirection::UPSTREAM, true, true, false};
